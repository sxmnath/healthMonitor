...
// ─── POST /ecg-live ────────────────────────────────────────────────────────────
// Lightweight, high-frequency sibling of POST /data — carries ONLY the raw
// ECG waveform, fired by the firmware the instant each ~1s sampling window
// finishes (every loop() iteration, not gated by the 5s vitals interval).
// Public, same as /data (device ingest, no auth). No MongoDB write here —
// pure relay, exactly like the waveform branch inside /data below.
//
// This replaced an earlier attempt at a persistent WebSocket connection from
// the ESP32 (a second TLS connection held open alongside the existing HTTPS
// client). That proved unstable on-device — plausibly memory pressure from
// running two live TLS sessions on an ESP32 at once, though it wasn't fully
// diagnosable without hardware access. A plain POST reusing the exact same
// HTTPClient mechanism that already works reliably for /data gets nearly
// the same latency win (every ~1-2s instead of every 5s) with far less risk.
app.post("/ecg-live", async (req, res) => {
  try {
    const { deviceId, ecgWaveform, ecgSampleRate } = req.body;
    if (!Array.isArray(ecgWaveform) || !ecgWaveform.length) {
      return res.status(204).end();
    }
    const patient = await Patient.findOne({ deviceId }).lean();
    const patient_id = patient ? patient.patient_id : deviceId;

    io.to(patient_id).emit("ecg-waveform", {
      patient_id,
    ...
    console.error("[POST /ecg-live]", err);
